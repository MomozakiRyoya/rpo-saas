...
1. **📖 [BETA_QUICKSTART.md](./BETA_QUICKSTART.md)** - ベータ版クイックスタート
2. **🚀 [BETA_DEPLOYMENT.md](./BETA_DEPLOYMENT.md)** - 詳細なデプロイ手順
3. **🔒 [BETA_SECURITY_NOTES.md](./BETA_SECURITY_NOTES.md)** - セキュリティ注意事項（必読）

### ベータ版の状態

✅ **利用可能な機能**
- 求人管理（作成・更新・削除）
- AI生成（テキスト: Claude / 画像: Gemini）
- 承認フロー
- コネクタ管理（Indeed, 求人ボックス, ダミー）
- メール送信（Resend）
- 非同期ジョブ処理（BullMQ）
- 問い合わせ対応・日程調整

⚠️ **制限事項**
- API Keyは平文保存（テスト用キーのみ使用推奨）
- Rate Limitingなし（少数顧客のみ）
- 本番APIキーの使用は非推奨
