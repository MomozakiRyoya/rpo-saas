...
export class ScoreCardController {
  ...
  async create(@Body() body: { name: string; jobId?: string; criteria: any[] }, @Request() req) {
  ...
  async addScore(
    @Param('id') id: string,
    @Body() body: { criteriaId: string; candidateId: string; score: number; comment?: string },
    @Request() req,
  ) {
  ...
  async getCandidateScores(@Param('candidateId') candidateId: string, @Request() req) {
    return this.scoreCardService.getCandidateScores(candidateId, req.user.tenantId);
  }
}
