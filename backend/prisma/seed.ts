...
async function main() {
  ...
  console.log('✅ Staff users created: admin, manager, member (password: password123)');

  // 顧客企業作成
  const customer1 = await prisma.customer.upsert({
    where: { id: 'seed-customer1-id-0001' },
    update: { name: '株式会社サンプルテック', description: 'IT企業、従業員100名' },
    create: {
  ...
  const customer2 = await prisma.customer.upsert({
    ...
    update: { name: '株式会社デモコーポレーション', description: '製造業、従業員500名' },
    create: {
  ...
  const customerUser = await prisma.user.upsert({
    ...
    update: { customerId: customer1.id },
    create: {
  ...
  console.log('✅ Customer portal user created: customer@demo.com (password: password123)');
